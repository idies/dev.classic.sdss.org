...
develdir=dr5web
pubdir=/afs/.fnal.gov/files/expwww/sdss/html/dr5
pubhost=flxi02
dmtag=frozen_for_dr3web
...
# old: public_loc="/data/dp7.a/DR1/doc"
#new AFS space, with some changes to do vol_release
...
print_usage () {
    ...
    echo
    echo " To publish on das.sdss.org/DR1, call -export first, then -publish"
    exit 1
}
...
export_dm () {
	...
	for directory in $dm_dirlist ; do
	    cvs export -r $tag -d ${expdir##*/} dm/$directory 
	done
	cd $cur
    else
	ln -s $expdir/dm dm
...
#old echo "scping $exp_dir/ to $public_loc"
#old scp -r ${exp_dir}/* ${public_loc}
    if [ -n $1 ]; then
	if [ "$1" = "private" ]; then
	    # Devel location is private anyway 
	    mv ${exp_dir}/.htaccess ${exp_dir}/htaccess
	else
	    # For public release, remove htaccess from public site
